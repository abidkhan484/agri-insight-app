...
- [ ] `SUPABASE_SERVICE_KEY` in `.env` only — never logged or returned in API response
- [ ] Frontend map uses `VITE_SUPABASE_ANON_KEY` (read-only) — correctly separated from service key
- [ ] Coordinate bounds validation (Bangladesh: lat 20.5–26.7N, lon 88.0–92.7E) at service layer
- [ ] Winston logger in all bot command handlers — no `console.log`
- [ ] All farmer-facing Telegram messages include Bangla (primary) + English (secondary)
- [ ] `docs/farmer-guide-bn-en.md` updated with `/joinmap` and `/faq` instructions
